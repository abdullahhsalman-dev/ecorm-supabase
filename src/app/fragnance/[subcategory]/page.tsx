...
export async function generateMetadata({
  ...
  return {
    title: `${subcategory.name} Fragrances | Diners`,
    description:
      subcategory.description ||
      `Shop our collection of ${subcategory.name.toLowerCase()} fragrances.`,
  };
}
...
export default async function FragranceSubcategoryPage({
  params,
}: {
  params: { subcategory: string };
}) {
    ...
    <div className="container px-4 py-8 md:py-12">
      <div className="mb-8">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>/</span>
          <Link href="/fragrance" className="hover:text-foreground">
            Fragrance
          </Link>
          <span>/</span>
          <span className="text-foreground">{subcategory.name}</span>
        </div>
        <h1 className="mt-4 text-3xl font-bold">{`${subcategory.name} Fragrances`}</h1>
        {subcategory.description && (
