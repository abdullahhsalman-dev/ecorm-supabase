 ...
 * values in state and calls validateCategoryForm on submit.
 *
...
export const categoryFormValues = (
...
export function availableParents(
  ...
  return roots.filter(
    (category) =>
      ...
      !isDescendant(categories, editingId, category.id),
  );
}
...
type ValidationResult =
  | { payload: CategoryPayload; error?: never }
...
export function validateCategoryForm(
  ...
  const duplicateSlug = categories.some(
    (category) =>
      category.slug.toLowerCase() === slug && category.id !== editingId,
  );

  if (duplicateSlug) {
    return invalid(
      "Slug already exists",
      "Please choose a different slug for this category.",
    );
  }
