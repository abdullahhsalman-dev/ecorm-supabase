...
export async function generateMetadata({
  ...
  return {
    title: `${subcategory.name} Winter Wear | Diners`,
    description:
      subcategory.description ||
      `Shop our collection of ${subcategory.name.toLowerCase()} winter wear.`,
  };
}
...
async function getSubcategory(slug: string) {
  try {
    ...
    return {
      ...
      slug: slug,
      description: `Shop our collection of ${slug.replace(
        /-/g,
        " "
      )} winter wear.`,
      parent_slug: "winter-wear",
    };
  } catch (error) {
...
export default async function WinterWearSubcategoryPage({
  params,
}: {
  params: { subcategory: string };
}) {
    ...
    <div className="container px-4 py-8 md:py-12">
      <div className="mb-8">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>/</span>
          <Link href="/winter-wear" className="hover:text-foreground">
            Winter Wear
          </Link>
          <span>/</span>
          <span className="text-foreground">{subcategory.name}</span>
        </div>
        <h1 className="mt-4 text-3xl font-bold">{`${subcategory.name} Winter Wear`}</h1>
        {subcategory.description && (
