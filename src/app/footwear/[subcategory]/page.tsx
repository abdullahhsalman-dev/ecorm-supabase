...
export async function generateMetadata({
  ...
  return {
    title: `${subcategory.name} Footwear | Diners`,
    description:
      subcategory.description ||
      `Shop our collection of ${subcategory.name.toLowerCase()} footwear.`,
  };
}
...
export default async function FootwearSubcategoryPage({
  params,
}: {
  params: { subcategory: string };
}) {
    ...
    <div className="container px-4 py-8 md:py-12">
      <div className="mb-8">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>/</span>
          <Link href="/footwear" className="hover:text-foreground">
            Footwear
          </Link>
          <span>/</span>
          <span className="text-foreground">{subcategory.name}</span>
        </div>
        <h1 className="mt-4 text-3xl font-bold">{`${subcategory.name} Footwear`}</h1>
        {subcategory.description && (
