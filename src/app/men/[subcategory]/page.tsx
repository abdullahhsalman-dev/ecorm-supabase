...
export async function generateMetadata({
  ...
  return {
    title: `${subcategory.name} | Men's Collection | Diners`,
    description:
      subcategory.description ||
      `Shop our collection of men's ${subcategory.name.toLowerCase()}.`,
  };
}
...
async function getSubcategory(slug: string) {
  try {
    ...
    return {
      ...
      slug: slug,
      description: `Shop our collection of men's ${slug.replace(/-/g, " ")}.`,
      parent_slug: "men",
    };
  } catch (error) {
...
export default async function MenSubcategoryPage({
  params,
}: {
  params: { subcategory: string };
}) {
    ...
    <div className="container px-4 py-8 md:py-12">
      <div className="mb-8">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>/</span>
          <Link href="/men" className="hover:text-foreground">
            Men
          </Link>
          <span>/</span>
          <span className="text-foreground">{subcategory.name}</span>
        </div>
        <h1 className="mt-4 text-3xl font-bold">{`Men's ${subcategory.name}`}</h1>
        {subcategory.description && (
