...
async function getSubcategory(slug: string) {
  try {
    ...
    return {
      ...
      slug: slug,
      description: `Shop our collection of women's ${slug.replace(/-/g, " ")}.`,
      parent_slug: "women",
    };
  } catch (error) {
...
export default async function WomenSubcategoryPage({
  params,
}: {
  params: { subcategory: string };
}) {
    ...
    <div className="container px-4 py-8 md:py-12">
      <div className="mb-8">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>/</span>
          <Link href="/women" className="hover:text-foreground">
            Women
          </Link>
          <span>/</span>
          <span className="text-foreground">{subcategory.name}</span>
        </div>
        <h1 className="mt-4 text-3xl font-bold">{`Women's ${subcategory.name}`}</h1>
        {subcategory.description && (
